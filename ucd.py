        ...
        if file.endswith('.zip'):
            ...
            if not os.path.exists(dst_folder):
                try:
                    ...
                    logger.info('Папка назначения была создана по пути указанному в конфиге')
                except OSError as e:
                    logger.exception('Ошибка создания папки назначения по пути указанному в конфиге.')
                    try:
                        shutil.rmtree(src_folder)
                        logger.info('Папка с новыми драйверами удалена')
                    except Exception as e:
                        logger.exception('Папка с новыми драйверами не может быть удалена')
                    logger.info('uzip_rename_move завершил работу')
                    logger.info('Драйверы НЕ обновлены')
                    sys.exit(1)
